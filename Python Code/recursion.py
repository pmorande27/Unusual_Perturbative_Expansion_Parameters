...
def plot(n,option):
    V,p0,Eminustwo,var,value,N = potential(option)
    ...
    for j in range(n):
        ...
        if j == 0:
            ...
            Elist[j] = collect(Elist[j-1] + Enminusone(j-1,V,p0,var,value,N)[1],N)
        print(j)
    return(n_list,Elist)
def potential(option):
    ...
    V = (1/(8*p**2) + potential)
    Eminustwo = V.subs(p,p0)*N
    return (V,p0,Eminustwo,var,value,N)
